...
def run_pca_and_models_cv(stage_name, X_sub_clean_df, y_sub, pca_num, label_a, label_b, initial_scaler=None):
    ...
    plt.savefig(f"results/{label_a}_vs_{label_b}_{stage_name}_({pca_num}_dimensons).png", dpi=300,
                bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        os.makedirs("results", exist_ok=True)
        summary_df.to_csv(f"results/model_comparison_summary_{pca_num}_dimensions.csv", index=False)
        print(f"\nSaved all results into: results/model_comparison_summary_{pca_num}_dimensions.csv")

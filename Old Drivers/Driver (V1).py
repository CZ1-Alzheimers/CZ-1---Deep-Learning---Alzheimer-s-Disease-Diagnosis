...
def test_classical_models_cv(X_sub, y_sub):
    ...
    for name, (model, param_grid) in models_and_params.items():
        ...
        all_fold_metrics = []
        for fold, (train_index, test_index) in enumerate(kf.split(X_sub, y_sub)):
            ...
            y_proba_fold = best_model.predict_proba(X_test_fold)[:, 1] if hasattr(best_model,
                                                                                  "predict_proba") else y_pred_fold
...
# PCA / Model Training
def run_pca_and_models_cv(stage_name, X_sub_clean_df, y_sub, pca_num, label_a, label_b, initial_scaler=None):
    print(f"\n--- PCA ({stage_name}) ---")
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X_final_df, y_sub)):
    ...
    plt.savefig(f"results/PCA - {pca_num}/{label_a}_vs_{label_b}_{stage_name}_({pca_num}_dimensons).png", dpi=300,
                bbox_inches='tight')
    plt.close()

    # Collect NN results
    nn_results = [
        [f"{label_a} vs {label_b}", stage_name, "NeuralNet",
    ...
    classical_cv_results = test_classical_models_cv(X_final_df, y_sub)

    # Collect all results
    all_results = nn_results
    for name, acc_m, acc_s, auc_m, auc_s, sens_m, sens_s, spec_m, spec_s, f1_m, f1_s in classical_cv_results:
        all_results.append([f"{label_a} vs {label_b}", stage_name, name,
...
if __name__ == "__main__":
    ...
    try:
        # Load dataset
        df = pd.read_csv("/Datasets/ADNI_dataset.csv")
        ...
        os.makedirs("results", exist_ok=True)
        summary_df.to_csv(f"results/PCA - {pca_num}/model_comparison_summary_{pca_num}_dimensions.csv", index=False)
        print(f"\nSaved all results into: results/PCA - {pca_num}/model_comparison_summary_{pca_num}_dimensions.csv")

    except FileNotFoundError:
        print("Error: Could not find 'ADNI_dataset.csv'. Please check your file path.")
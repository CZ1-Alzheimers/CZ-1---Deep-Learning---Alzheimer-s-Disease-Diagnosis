...
def test_classical_models_cv(X_sub, y_sub):
    ...
    # 5-Fold Stratified Cross-Validation setup to ensure both classes in each fold
...
def run_pca_and_models_cv(stage_name, X_sub_clean_df, y_sub, pca_num, label_a, label_b, initial_scaler=None):
    ...
    plt.savefig(f"R/PCA - {pca_num}/{label_a}_vs_{label_b}_{stage_name}_({pca_num}_dimensons).png", dpi=300,
                bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Error: Could not find dataset. Please check your file path.")
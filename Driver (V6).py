...
def run_pca_and_models_cv(stage_name, X_sub_clean_df, y_sub, groups, pca_num, code_a, code_b, label_a, label_b):
    ...
    classical_cv_results, classical_roc_data = test_models(
        X_sub_clean_df, y_sub, groups, results_path, code_a, code_b, label_a, label_b, stage_name, pca_num
    )
...
def binary_compare(data_row_series, comparison_label, pca_dim, results_path):
    ...
    try:
        ...
        print(f"Error: Required file not found: {e.filename}. Skipping prediction.")
        return None, None
    ...
    # The pipeline handles scaling, PCA, and prediction sequentially
...
if __name__ == "__main__":
    ...
    try:
        ...
        os.makedirs(results_path, exist_ok=True)
        summary_filename = f"{results_path}/model_comparison_summary_{pca_num}_dimensions_Pipeline.csv"
        summary_df.to_csv(summary_filename, index=False)
        summary_df.to_csv("model_comparison_summary.csv", index=False)
        print(f"\nSaved all results into: {summary_filename}")

        # Test multiple images from a CSV file
        test_filename = "Datasets/Aligned Datasets/Oasis3_aligned_data.csv"
        ...
        try:
            ...
            for index, row in test_df.iterrows():
                ...
                # Perform 3-way prediction (returns string: 'NC', 'MCI', or 'AD')
                final_pred_str = final_diagnosis(
            ...
            test_output_filename = f"Results/{data}/PCA - {pca_num}/Multi_Subject_Prediction_Summary_Pipeline.csv"
